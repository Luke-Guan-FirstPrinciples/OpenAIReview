...
3. Model-prefix match: `anthropic/Codex-*` → Anthropic API if key available
...
### Codex Skill (`src/reviewer/skill/`)
- `SKILL.md` — Skill definition for `/openaireview` command in Codex. Runs a multi-agent pipeline with section-level sub-agents and cross-cutting agents, producing severity-tiered findings (major/moderate/minor).
...
- Install with `openaireview install-skill`, then use `/openaireview <path-or-url>` in Codex.
